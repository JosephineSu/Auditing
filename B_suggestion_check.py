    ...
    for row in tableB.iterrows():
        ...
        if single_row['B101'] == 1:
            ...
            if single_row['B103'] == 4 and single_row['B102'] != 2 and single_row['B102'] != 7 and single_row['B102'] != 8:
                ...
                dict['核实内容'] = "竹草土坯结构的房屋是楼房或单元房吗？请核实"
                insert_to_pd(dict)
...
if __name__ == "__main__":
    ...
    B_suggestion_data = {'year': [], 'sid': [], 'scode': [], 'code': [], '核实内容': [],'核实说明':[], 'townname': [], 'vname': []}
    B_suggestion_result = pd.DataFrame(B_suggestion_data)
    B_suggestion_result = B_suggestion_result[['year', 'sid', 'scode', 'code', '核实内容','核实说明', 'townname', 'vname']]
