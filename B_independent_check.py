...
def B_independent_check(tableB,xiaoqu,result):
    ...
    for row in tableB.iterrows():
        ...
        dict = {'year': year, 'sid': family_sid, 'scode': scode,'核实说明':None, 'townname': townname,'vname': vname}

        if B['B102'] == 8:
            dict['code'] = "B102=8"
            dict['核实内容'] = "本户居住空间样式为其他？请核实！"
            insert_to_pd(dict)

        if B['B103'] == 4 or B['B103'] == 5:
            dict['code'] = "B103={}".format(B['B103'])
            dict['核实内容'] = "本户房屋主要建筑材料为竹草或其他？请核实！"
            insert_to_pd(dict)
        ...
        if b104 < 3 or b104 > 8:
            ...
            for i in arr:
                if B[i] != 0 and pd.isnull(B[i])==False:
                    ...
                    dict['核实内容'] = "请查看B卷B117-B126的值,本户B104不为3-8，B117-B126却有值。填错？"
                    insert_to_pd(dict)

        if (b104 == 1 or b104 == 2) and B['B127'] == 0:
            dict['code'] = "B127={},B104={}".format(B['B127'], b104)
            dict['核实内容'] = "本户租赁房户，但却没有租赁月租金?请核实！"
            insert_to_pd(dict)
        if (b104 != 1 and b104 != 2) and B['B127'] != 0 and pd.isnull(B['B127'])==False:
            dict['code'] = "B127={},B104={}".format(B['B127'], b104)
            dict['核实内容'] = "本户租赁月租金B127有值，但却不是租赁房（B104不为1或2），填错?请核实！"
            insert_to_pd(dict)

        if 0 < B['B150'] < 0.5:
            dict['code'] = "B150={}".format(B['B150'])
            dict['核实内容'] = "本户期内住房大修或装修费用低于5000元或高于30万元？"
            insert_to_pd(dict)

        if B['B238'] == 0:
            dict['code'] = "B238={}".format(B['B238'])
            dict['核实内容'] = "本户现住房所处场地B238(X21)为空，漏填？"
            insert_to_pd(dict)
        else:
            if B['B238'] != 4:
                dict['code'] = "B238={}".format(B['B238'])
                dict['核实内容'] = "本户现住房所处场地B238(X21)有危险？请核实！"
                insert_to_pd(dict)

        if B['B239'] == 0:
            dict['code'] = "B239={}".format(B['B239'])
            dict['核实内容'] = "本户现住房安全状况B239(X22)为空，漏填？"
            insert_to_pd(dict)
        else:
            if B['B239'] != 1 and B['B239'] != 2:
                dict['code'] = "B239={}".format(B['B239'])
                dict['核实内容'] = "本户现住房安全状况B239(X22)有危险？请核实！"
                insert_to_pd(dict)

        if B['B240'] == 0:
            dict['code'] = "B240={}".format(B['B240'])
            dict['核实内容'] = "本户住宅地面经常有泥土、沙土、畜禽粪便等脏东西B240(X23)为空，漏填？请核实！"
            insert_to_pd(dict)
        else:
            if B['B240'] != 2:
                dict['code'] = "B240={}".format(B['B240'])
                dict['核实内容'] = "本户住宅地面经常有泥土、沙土、畜禽粪便等脏东西？B240(X23)请核实！"
                insert_to_pd(dict)
...
# ******下面是B卷超界的审核****************************************************
        for i in ['B101','B109','B113','B238','B239']:
            ...
            if bi != 1 and bi != 2 and bi != 3 and bi != 4:
                ...
                dict['核实内容'] = "本户{}所填内容超界或漏填。请核实！".format(bi)
                insert_to_pd(dict)
        ...
        for j in ['B106','B107','B112','B114']:
            ...
            if bj != 1 and bj != 2 and bj != 3:
                ...
                dict['核实内容'] = "本户{}所填内容超界或漏填。请核实！".format(j)
                insert_to_pd(dict)
        ...
        for k in ['B240','B241','B243']:
            ...
            if bk != 1 and bk != 2:
                ...
                dict['核实内容'] = "本户{}所填内容超界或漏填。请核实！".format(k)
                insert_to_pd(dict)
        ...
        for m in ['B102']:
            ...
            if bm != 1 and bm != 2 and bm != 3 and bm!=4 and bm!=5 and bm!=6 and bm!=7 and bm!=8:
                ...
                dict['核实内容'] = "本户{}所填内容超界或漏填。请核实！".format(m)
                insert_to_pd(dict)
        ...
        if b116 != 1 and b116 != 2 and b116 != 3 and b116 != 116 and b116 != 5 and b116 != 5 and b116 != 6 and b116 != 7 and b116 != 8 and b116 != 11:
            ...
            dict['核实内容'] = "本户B116所填内容超界或漏填。请核实！"
            insert_to_pd(dict)

        b126 = B['B126']
        if b126!= 1 and b126!=2 and b126!=0:
            dict['code'] = "B126={}".format(b126)
            dict['核实内容'] = "本户B126所填内容超界或漏填。请核实！"
            insert_to_pd(dict)
...
if  __name__ == "__main__":
    ...
    head = {'year': [], 'sid': [], 'scode': [], 'code': [], '核实内容': [],'核实说明':[], 'townname': [], 'vname': []}
    B_independent_result = pd.DataFrame(head)
    B_independent_result = B_independent_result[['year', 'sid', 'scode', 'code', '核实内容','核实说明', 'townname', 'vname']]
